...
import './PetSitterServiceData.css';
...
const PetSitterServiceData = () => {
    ...
    const handleToggleStatus = async (serviceName, currentStatus) => {
        try {
            setUpdatingStatus(true); // start spinner
            await axios.patch(`http://localhost:8080/api/pet-services/${serviceName}`, {
                req_accepted: !currentStatus
            });
            console.log("Service status updated:", serviceName, currentStatus ? "Pending" : "Done");
            fetchProfile(); // refresh profile data
        } catch (error) {
            console.error('Error updating service status:', error);
        } finally {
            setUpdatingStatus(false); // stop spinner
        }
    };
            ...
            <div className="services-section">
                <div className="services-card">
                    ...
                    <button
                        ...
                        onClick={() => handleToggleStatus(profile.name, profile.req_accepted)}
                        disabled={updatingStatus}
                    >
                        {updatingStatus ? (
                            <span className="spinner"></span>
                        ) : (
                            profile.req_accepted === true ? "accepted" : "pending"
                        )}
                    </button>
...
export default PetSitterServiceData;